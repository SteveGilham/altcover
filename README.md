...
```AltCover Runner [/r[ecordingDirectory]=VALUE] [/w[orkingDirectory]=VALUE] [/x|executable=VALUE] [/?|h[elp]] [-- arguments]```
...
*  -r, --recorderDirectory=VALUE The folder containing the instrumented code to monitor (including the AltCover.Recorder.g.dll generated by previous a use of the .net core AltCover).
*  -w, --workingDirectory=VALUE  Optional: The working directory for the application launch
*  -x, --executable=VALUE        The executable to run e.g. `dotnet`
*  -?, --help, -h                Prints out the options.
* --                           the rest of the command line is treated as the arguments to the command to execute 

### Notes

In runner mode, the command being executed (`-x`) is mandatory.  Before the exectable is launched AltCover creates a file in the same folder as the previously nominated XML report, and while that file is present, the raw coverage data are written there, rather than being held in memory.  After the executable terminates, the data are read and processed into the XML report.
...
AltCover in its instrumenting mode, through the optional trailing arguments, can be used instrument and run tests all in one command line.  The runner-style coverage gathering isn't done in this mode, so this feature would be mainly of interest outside the .net core use case.  Of course the trailing arguments can be used to launch AltCover in runner-mode if  required.
