...
module Adapter =
  ...
  let internal prepareName name =
    if name
       |> Instance.Visits.[Instance.VisitIndex |> int].ContainsKey
       |> not
    then
      let entry = Dictionary<int, PointVisit>()
      Instance.Visits.[Instance.VisitIndex |> int].Add(name, entry)

  let VisitsAdd name line number =
    prepareName name
    let v = PointVisit.Init number []
    Instance.Visits.[Instance.VisitIndex |> int].[name].Add(line, v)
  ...
  let VisitsEntrySeq key = Instance.Visits.[Instance.VisitIndex |> int].[key] |> Seq.cast<obj>
  let VisitCount key key2 = (Instance.Visits.[Instance.VisitIndex |> int].[key].[key2]).Count
  let VisitTracks key key2 = (Instance.Visits.[Instance.VisitIndex |> int].[key].[key2]).Tracks |> Seq.cast<obj>
  let VisitTrack key key2 key3 =
    let raw = (Instance.Visits.[Instance.VisitIndex |> int].[key].[key2]).Tracks.[key3]
    match raw with
    | Null -> [| int64 Tag.Null |]
