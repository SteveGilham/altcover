...
let fake = Path.Combine  (cache, "fake") 
let fakelib = Directory.GetFiles(fake, "*akelib.dll", SearchOption.AllDirectories)
              |> Seq.last
              |> Path.GetDirectoryName
...
let formatted = String.Format(build, lintlib, mdlib, fakelib)
