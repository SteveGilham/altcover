# TypeSafe.DirectoryPath.Equals method (1 of 3)

```csharp
public bool Equals(DirectoryPath obj)
```

## See Also

* class [DirectoryPath](../TypeSafe.DirectoryPath-apidoc)
* namespace [AltCover](../../AltCover-apidoc)

---

# TypeSafe.DirectoryPath.Equals method (2 of 3)

```csharp
public bool Equals(object obj)
```

## See Also

* class [DirectoryPath](../TypeSafe.DirectoryPath-apidoc)
* namespace [AltCover](../../AltCover-apidoc)

---

# TypeSafe.DirectoryPath.Equals method (3 of 3)

```csharp
public bool Equals(object obj, IEqualityComparer comp)
```

## See Also

* class [DirectoryPath](../TypeSafe.DirectoryPath-apidoc)
* namespace [AltCover](../../AltCover-apidoc)

<!-- DO NOT EDIT: generated by xmldocmd for AltCover.exe -->
