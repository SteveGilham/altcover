...
let checkCalls (m: MethodDefinition) =
  ...
  |> Seq.filter (fun i ->
    let op = i.Operand :?> MethodReference

    op.Name = "Visit"
    && op.DeclaringType.FullName = "AltCover.Recorder.Instance")
  |> Seq.iter (fun v ->
    let p = v.Previous
    ...
    if p2.OpCode.ToString() <> "ldstr" then
      eprintfn "Suspicious call in %s - module id = %A" m.FullName p2

    if p2.Operand |> string |> String.IsNullOrWhiteSpace then
      eprintfn "Suspicious call in %s - module id = %A" m.FullName p2

    il
    |> Seq.iter (fun o ->
      match o.OpCode.OperandType with
      | OperandType.InlineBrTarget
      | OperandType.ShortInlineBrTarget ->
        let target = o.Operand :?> Instruction

        if target.Offset = v.Offset then
          eprintfn "Suspicious jump in %s from %A to %A" m.FullName o v
