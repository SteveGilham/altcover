...
let lint =  Path.Combine  (cache, "fsharplint.core")
let lintlib = Directory.GetFiles(lint, "*SharpLint.Core.dll", SearchOption.AllDirectories)
              |> Seq.filter (fun n -> n.Contains("netstandard"))
              |> Seq.last
...
let formatted = String.Format(build, lintlib)
