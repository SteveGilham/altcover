...
type AltCoverTests() = class
  ...
  member self.ShouldMatchGendarmeComplexityInSamples() =
    // Hack for running while instrumented
    let where = Assembly.GetExecutingAssembly().Location
    let path = Path.Combine(where.Substring(0, where.IndexOf("_Binaries")) + "_Binaries/Sample3/Debug+AnyCPU/netstandard2.0", "Sample3.dll")
    let def = Mono.Cecil.AssemblyDefinition.ReadAssembly path
    let failures = def.MainModule.Types
                    |> Seq.cast<TypeDefinition>
                    |> Seq.map Rocks.GetAllTypes
  ...
  member self.ShouldMatchGendarmeComplexityInMonoSamples() =
    // Hack for running while instrumented
    let where = Assembly.GetExecutingAssembly().Location
    let path0 = Path.Combine(where.Substring(0, where.IndexOf("_Binaries")) + "_Mono/Sample3", "Sample3.dll")
    let path = if File.Exists path0 then path0
               else Path.Combine(where.Substring(0, where.IndexOf("_Binaries")) + "../_Mono/Sample3", "Sample3.dll")
    let def = Mono.Cecil.AssemblyDefinition.ReadAssembly path
    let failures = def.MainModule.Types
                    |> Seq.cast<TypeDefinition>
                    |> Seq.map Rocks.GetAllTypes
