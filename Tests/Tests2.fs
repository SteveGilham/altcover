...
// TODO -- see Instrument.WriteAssembly       Assert.That (File.Exists (outputdll + ".mdb"))
    ...
    let ShouldBeAbleToInstrumentASwitchForNCover() =
      ...
      let m = Node.Method (target, Inspect.Instrument, None, 0)
    ...
    let ExcludedMethodShouldNotChangeState() =
      let input = InstrumentContext.Build []
      let output =
        Instrument.InstrumentationVisitor input (Node.Method(null, Inspect.Ignore, None, 0))
      Assert.That(output, Is.SameAs input)
    ...
    let IncludedMethodShouldChangeState() =
      ...
      let output =
        Instrument.InstrumentationVisitor input
          (Node.Method(func, Inspect.Instrument, None, 0))
    ...
    let ExcludedMethodPointIsPassThrough() =
      ...
      let def = Mono.Cecil.AssemblyDefinition.ReadAssembly path
      ProgramDatabase.ReadSymbols def
      let visited = Node.MethodPoint(null, None, 0, false, 0)
    ...
    let IncludedMethodPointInsertsVisit() =
      ...
      if File.Exists(pdb) then // skip when we don't have symbols on travis
        ...
        let visited = Node.MethodPoint(target, None, 32767, true, 0)
