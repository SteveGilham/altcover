# TypeSafe.ContextItem.Equals method (1 of 3)

```csharp
public bool Equals(ContextItem obj)
```

## See Also

* class [ContextItem](../TypeSafe.ContextItem-apidoc)
* namespace [AltCover](../../AltCover-apidoc)

---

# TypeSafe.ContextItem.Equals method (2 of 3)

```csharp
public bool Equals(object obj)
```

## See Also

* class [ContextItem](../TypeSafe.ContextItem-apidoc)
* namespace [AltCover](../../AltCover-apidoc)

---

# TypeSafe.ContextItem.Equals method (3 of 3)

```csharp
public bool Equals(object obj, IEqualityComparer comp)
```

## See Also

* class [ContextItem](../TypeSafe.ContextItem-apidoc)
* namespace [AltCover](../../AltCover-apidoc)

<!-- DO NOT EDIT: generated by xmldocmd for AltCover.exe -->
